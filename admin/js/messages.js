...
/* Convertit un doc Firestore en objet message */
function docToMsg(doc) {
  ...
  return {
    ...
    email:       d.email       || '',
    phone:       d.phone       || null,
    subject:     d.subject     || 'Demande de devis',
    message:     d.message     || '',
    plan:        d.plan        || null,
    status:      d.status      || 'new',
    source:      d.source      || 'site_contact',
    created_at:  d.created_at_iso || (d.created_at && d.created_at.toDate ? d.created_at.toDate().toISOString() : null) || new Date().toISOString(),
    read_at:     d.read_at     || null,
    replied_at:  d.replied_at  || null,
  };
}

/* Lance l'écoute temps réel Firestore */
function startFirestoreListener() {
  if (firestoreUnsubscribe) firestoreUnsubscribe();
  firestoreUnsubscribe = db.collection(MSG_KEY)
    .orderBy('created_at', 'desc')
    .onSnapshot(snapshot => {
      allMessages = snapshot.docs.map(docToMsg);
      updateNavBadge();
...
/* getContacts — retourne la copie locale déjà chargée */
function getContacts() { return allMessages; }

async function clearAllMessages() {
  if (!confirm('Supprimer tous les messages ? Cette action est irréversible.')) return;
  try {
    const snap = await db.collection(MSG_KEY).get();
...
function getConversationMessages(message, contacts = allMessages) {
...
function updateNavBadge() {
  const n     = allMessages.filter(c => c.status === 'new').length;
...
function loadMessages() {
  // Lance l'écoute temps réel Firestore (mise à jour automatique)
  startFirestoreListener();
  document.getElementById('msg-detail').innerHTML = renderMsgEmpty(
    'Chargement…', 'Récupération des messages en cours.'
  );
}
...
function renderMessageDetail(id, markRead = false) {
  ...
  if (markRead && m.status === 'new') {
    updateMsgStatus(id, 'read'); // async Firestore update
  }

  const history          = getConversationMessages(m, allMessages);
...
async function updateMsgStatus(id, status) {
  try {
    const update = { status };
    if (status === 'read' || status === 'replied') update.read_at = new Date().toISOString();
    if (status === 'replied') update.replied_at = new Date().toISOString();
    await db.collection(MSG_KEY).doc(id).update(update);
...
async function deleteMsg(id) {
  ...
  try {
    await db.collection(MSG_KEY).doc(id).delete();
    if (activeMsg === id) {
      activeMsg = null;
      document.getElementById('msg-detail').innerHTML = renderMsgEmpty('Message supprimé', 'Sélectionnez un autre lead.');
    }
...
/* ── Temps réel géré par Firestore onSnapshot (voir startFirestoreListener) ── */
/* L'écoute localStorage n'est plus nécessaire avec Firestore */
