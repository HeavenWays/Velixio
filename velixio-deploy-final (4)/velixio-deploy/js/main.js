    ...
    // ✦.✦✦.✦✦.✦ FORMULAIRE CONTACT — sauvegarde localStorage ✦.✦✦.✦✦.✦
    function sendForm(e) {
      e.preventDefault();
      ...
      var prenom  = (document.getElementById('cf-prenom')  || { value: '' }).value.trim();
      var nom     = (document.getElementById('cf-nom')     || { value: '' }).value.trim();
      var email   = (document.getElementById('cf-email')   || { value: '' }).value.trim();
      var tel     = (document.getElementById('cf-tel')     || { value: '' }).value.trim();
      var projet  = (document.getElementById('cf-projet')  || { value: '' }).value.trim();
      var plan    = (document.getElementById('cf-plan')    || { value: '' }).value.trim();
      var msg     = (document.getElementById('cf-msg')     || { value: '' }).value.trim();
      ...
      try {
        ...
        var contacts = JSON.parse(localStorage.getItem('velixio_contacts') || '[]');
        ...
        contacts.unshift(newContact); // Ajouter en tête
        localStorage.setItem('velixio_contacts', JSON.stringify(contacts));
        ...
        console.error('Erreur sauvegarde:', ex);
        btn.textContent = 'Envoyer ma demande →'; btn.disabled = false;
        err.innerHTML = 'Erreur — <a href="https://wa.me/212674323871" style="color:#f87171">WhatsApp ↗</a>';
